...
import com.niit.chatonbackend.service.BlogCommentService;
...
public class BlogCommentController {
	...
	public ResponseEntity<List<BlogComment>> getCommentById(@PathVariable("blogId") String blogId) {
		List<BlogComment> comments = blogCommentService.getCommentById(blogId);
	...
	public ResponseEntity<BlogComment> createComment(@RequestBody BlogComment comment,HttpSession session, UriComponentsBuilder build) {
		Date date = new Date();
		...
		return new ResponseEntity<BlogComment>(headers, HttpStatus.CREATED);
	}
